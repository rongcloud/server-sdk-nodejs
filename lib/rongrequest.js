...
function requestWithDomain ( base_url, api, params, format, callback ) {
	var f = 'json';
	...
	.type( 'form' )
	.send( params )
	.end( promised_callback(callback) );
}

function promised_callback (callback) {
	return function ( err, result ) {
		if( err ) {
			return Promise.reject(err).asCallback(callback)
		}
		else {
			return Promise.resolve(result.text).asCallback(callback)
		}
	}
}
...
exports.requestWithSameFields = function( api, params, fieldAndValues, format, callback ) {
  ...
  agent.end( promised_callback(callback) );
}
