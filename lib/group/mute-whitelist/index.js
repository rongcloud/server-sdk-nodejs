...
let add = (group) => {
  ...
  let whVerify = _.extend(_.pick(verify.group, 'id', 'members'));
  let error = check({
    api: conf,
    model: 'group',
    data: group,
...
let remove = (group) => {
  ...
  let error = check({
    ...
    model: 'group',
    data: group,
    verify: _.pick(verify.group, 'id', 'members')
  });
  if (error) {
    return Promise.reject(error);
  }
  ...
  return request({
    ...
    logger.log({
      content: error,
      level: 'error',
      pos: 'Group.MuteWhiteList.remove'
    });
    return error;
  });
};
...
let getList = (group) => {
  ...
  return request({
    ...
    result = _.omit(result, 'userIds')
    return _.extend(result, {
      id: group.groupId,
      members: _.map(members, (memberId) => {
        return {
          id: memberId
        }
      })
    ...
    logger.log({
      content: error,
      level: 'error',
      pos: 'Group.MuteWhiteList.getList'
    });
    return error;
  });
};
